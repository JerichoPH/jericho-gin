...
func (receiver *ExcelReader) AutoRead(filename string, a ...interface{}) *ExcelReader {
	return receiver.
		OpenFile(filename, a...).
		SetOriginalRow(2).
		SetTitleRow(1).
...
func (receiver *ExcelReader) ToMap() map[uint64]map[string]string {
	...
	for rowNumber, row := range receiver.ToList() {
		if len(receiver.GetTitle()) != len(row) {
			wrongs.ThrowForbidden(fmt.Sprintf("表头数量与实际数据列不匹配（第%d行）", rowNumber))
		}
...
// OpenFile 打开文件
func (receiver *ExcelReader) OpenFile(filename string, more ...interface{}) *ExcelReader {
	if filename == "" {
		wrongs.ThrowEmpty("文件名不能为空")
	}
	f, err := excelize.OpenFile(fmt.Sprintf(filename, more...))
	if err != nil {
		wrongs.ThrowForbidden(fmt.Sprintf("打开文件错误：%s", err.Error()))
	}
	receiver.excel = f

	defer func() {
		if err := receiver.excel.Close(); err != nil {
			wrongs.ThrowForbidden("文件关闭错误")
		}
	}()
...
func (receiver *ExcelReader) ReadTitle() *ExcelReader {
	if receiver.GetSheetName() == "" {
		wrongs.ThrowEmpty("未设置工作表名称")
	}

	if rows, err := receiver.excel.GetRows(receiver.GetSheetName()); err != nil {
		wrongs.ThrowForbidden("读取数据错误" + err.Error())
	} else {
...
// NewExcelWriter 初始化
func NewExcelWriter(filename string, a ...interface{}) *ExcelWriter {
...
func (receiver *ExcelWriter) setStyleFont(cell *ExcelCell) {
	if style, err := receiver.excel.NewStyle(&excelize.Style{
		...
		wrongs.ThrowForbidden(fmt.Sprintf("设成字体错误：%s", cell.GetCoordinate()))
	} else {
		if err = receiver.excel.SetCellStyle(receiver.sheetName, cell.GetCoordinate(), cell.GetCoordinate(), style); err != nil {
			wrongs.ThrowForbidden(fmt.Sprintf("设置字体错误：%s", cell.GetCoordinate()))
		}
	}
}
	...
	for _, cell := range excelRow.GetCells() {
		...
		case ExcelCellContentTypeFormula:
			if err := receiver.excel.SetCellFormula(receiver.sheetName, cell.GetCoordinate(), cell.GetContent().(string)); err != nil {
				wrongs.ThrowForbidden("写入数据错误（公式）%s：%v", cell.GetCoordinate(), cell.GetContent(), err.Error())
			}
		case ExcelCellContentTypeInt:
			if err := receiver.excel.SetCellInt(receiver.sheetName, cell.GetCoordinate(), cell.GetContent().(int)); err != nil {
				wrongs.ThrowForbidden("写入数据错误（数字）%s：%v", cell.GetCoordinate(), cell.GetContent(), err.Error())
			}
		case ExcelCellContentTypeFloat64:
			if err := receiver.excel.SetCellFloat(receiver.sheetName, cell.GetCoordinate(), cell.GetContent().(float64), 4, 64); err != nil {
				wrongs.ThrowForbidden("写入数据错误（小数）%s：%v", cell.GetCoordinate(), cell.GetContent(), err.Error())
			}
		case ExcelCellContentTypeBool:
			if err := receiver.excel.SetCellBool(receiver.sheetName, cell.GetCoordinate(), cell.GetContent().(bool)); err != nil {
				wrongs.ThrowForbidden("写入数据错误（布尔）%s：%v", cell.GetCoordinate(), cell.GetContent(), err.Error())
			}
		default:
			if err := receiver.excel.SetCellValue(receiver.sheetName, cell.GetCoordinate(), cell.GetContent()); err != nil {
				wrongs.ThrowForbidden("写入数据错误（默认）%s：%v", cell.GetCoordinate(), cell.GetContent(), err.Error())
			}
		}
...
func (receiver *ExcelRow) SetCells(cells []*ExcelCell) *ExcelRow {
	if receiver.GetRowNumber() == 0 {
		wrongs.ThrowForbidden("行标必须大于0")
	}

	for colNumber, cell := range cells {
		if colText, err := excelize.ColumnNumberToName(colNumber + 1); err != nil {
			wrongs.ThrowForbidden(fmt.Sprintf("列索引转列文字失败：%d，%d", receiver.GetRowNumber(), colNumber+1))
		} else {
...
func NewExcelCellAny(content interface{}) *ExcelCell {
...
// Init 初始化
func (receiver *ExcelCell) Init(content interface{}) *ExcelCell {
	receiver.content = content
	return receiver
}

// GetContent 获取内容
func (receiver *ExcelCell) GetContent() interface{} {
	return receiver.content
}

// SetContent 设置内容
func (receiver *ExcelCell) SetContent(content interface{}) *ExcelCell {
	receiver.content = content
	return receiver
}
